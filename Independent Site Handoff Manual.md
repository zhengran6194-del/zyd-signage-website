...
首页工厂实况目前已接入视频：`public/assets/videos/factory-live.mp4`，代码位置为 `src/app/page.tsx`；视频 poster 为 `public/assets/images/factory-video-poster.jpg`。以后替换视频时，请用相同文件名覆盖 MP4，测试后通过 GitHub Desktop 提交并 Push。若要修改视频路径、poster 或播放行为，再同步修改首页 `<video>` 节点。详细步骤请查阅 [IMAGE_SWAP_GUIDE.md](IMAGE_SWAP_GUIDE.md) 的“Factory Live Section: Video Connected”章节。
